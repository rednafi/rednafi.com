...
I've been dabbling with Hynek's [stamina] for a while. It's a Python library for retrying
flaky service calls, built on top of the battle-tested [tenacity]. It has saner defaults and
some neat hooks for testing.

One of the neat retry patterns I learned from reading the source code is how you can use a
context manager in a for-loop to retry a block of code. If you're writing a library that
handles retries, you can't add exception handling with `try...except` within a user-written
for-loop. Using a context manager for this is neat. It allows you to write code like this:

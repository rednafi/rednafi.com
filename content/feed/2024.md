...
While I don’t get to write much Go at work these days, I enjoy using it to build my tools
and have been doing so for the past few years. Go doesn’t need much evangelizing anymore,
but there’s one talk I often recommend because it perfectly captures why writing a server in
Go makes so much sense.

Sameer introduces a toy search server and gradually makes it concurrent as the talk
progresses. It showcases goroutines, buffered and unbuffered channels, and waiting on
multiple channels with `select`, all in a single, coherent example.
